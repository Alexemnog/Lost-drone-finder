...
class LostDroneFinder(tk.Tk):
    ...
    def _add_info_panel(self, parent, title, lines):
        box = ttk.LabelFrame(parent, text=title, style="Card.TLabelframe", padding=14)
        box.pack(fill="both", expand=True, pady=(14, 0))
        for line in lines:
            ttk.Label(box, text=line, style="Muted.TLabel", wraplength=720).pack(anchor="w", pady=4)
        return box
    ...
    def _step_one_content(self, parent):
        ...
        self._add_info_panel(
            scenario,
            "Какво ще използва изчислението",
            (
                "1. Типът на дрона променя поведението след загуба на сигнал.",
                "2. При FPV бавно кацане дронът продължава да се носи леко по вятъра.",
                "3. При нормален дрон зоната остава близо, освен ако вятърът е силен.",
            ),
        )
    ...
    def _weather_section(self, parent):
        ...
        self._add_info_panel(
            box,
            "Проверка преди изчисление",
            (
                "Последната позиция трябва да е възможно най-точна.",
                "Миналото време след загуба е в секунди.",
                "Ако не знаеш реалния вятър, остави автоматичния режим.",
            ),
        )
